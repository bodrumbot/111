...
// Joylashuv ko'rsatish bilan yangilangan
// ==========================================
...
function init() {
  console.log('🚀 Admin panel init (Faqat qabul qilinganlar + Joylashuv)');
...
window.openOrderModal = async function(orderId) {
  ...
  if (locationRow && modalLocation) {
    if (order.location) {
      // Location format: "lat,lng" yoki manzil matni
      if (order.location.includes(',')) {
        const [lat, lng] = order.location.split(',');
        const trimmedLat = lat.trim();
        const trimmedLng = lng.trim();
        
        modalLocation.href = `https://maps.google.com/?q=${trimmedLat},${trimmedLng}`;
        modalLocation.innerHTML = `📍 Xaritada ko'rish →<br><small style="color: #888; font-size: 12px;">${parseFloat(trimmedLat).toFixed(5)}, ${parseFloat(trimmedLng).toFixed(5)}</small>`;
        locationRow.style.display = 'flex';
        
        // Xarita iframe yoki rasm qo'shish (ixtiyoriy)
        // showMapPreview(trimmedLat, trimmedLng);
      } else {
        // Matnli manzil
        modalLocation.href = '#';
        modalLocation.textContent = order.location;
        modalLocation.onclick = (e) => {
          e.preventDefault();
          alert('Manzil: ' + order.location);
        };
        locationRow.style.display = 'flex';
      }
    } else {
      ...
      ${customerOrders.slice(0, 10).map((o, i) => {
        ...
        <div style="background: rgba(255,255,255,0.03); padding: 12px; border-radius: 12px; border: 1px solid #333; animation: slideInRight 0.3s ease ${i * 0.05}s backwards;">
          <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
            <span style="font-weight: 600;">${hasLoc}${hasScr}#${(o.order_id || o.orderId)?.slice(-6)}</span>
          ...
          ${o.location ? `<div style="font-size: 11px; color: #3498db; margin-top: 4px;">📍 Joylashuv: ${o.location.includes(',') ? 'Xaritada' : o.location}</div>` : ''}
        </div>
      `}).join('')}
    </div>
  `;
...
style.textContent = `
  ...
  /* Xarita preview (agar kerak bo'lsa) */
  .map-preview {
    margin: 16px 0;
    border-radius: 12px;
    overflow: hidden;
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('🚀 DOMContentLoaded - Admin (Faqat qabul qilinganlar + Joylashuv)');
  init();
});
